...
type MelkeyGoStackProps struct {
	awscdk.StackProps
}

func NewMelkeyGoStack(scope constructs.Construct, id string, props *MelkeyGoStackProps) awscdk.Stack {
	...
	api := awsapigateway.NewRestApi(stack, jsii.String("myAPIGateway"), &awsapigateway.RestApiProps{
		...
		// Set up AWS CloudWatch log or comment "DeployOptions" out to run successfully.
...
func main() {
	...
	NewMelkeyGoStack(app, "MelkeyGoStack", &MelkeyGoStackProps{
		awscdk.StackProps{
			Env: env(),
		},
	})
